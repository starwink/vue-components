<template>
    <Modal v-model="show" title="二维码管理" :mask-closable="false" width="90%" class-name="form">
        <div class="admin-page">
            <Card class="search_where">
                <Form :model="form" ref="searchForm" :label-width="100" @submit.native.prevent @keydown.native.enter.prevent="search"> 
                    <Row :gutter="32">
                        <Col span="6">
                        ...
                        <Col span="12">
                        <FormItem label="创建时间2：">
                            <DatePicker v-model="form.createTimeStart" type="datetime" format="yyyy-MM-dd HH:mm" placeholder="请选择开始时间" style="width: calc(50% - 10px)"></DatePicker>
                            <DatePicker v-model="form.createTimeEnd" type="datetime" format="yyyy-MM-dd HH:mm" placeholder="请选择结束时间" style="width: calc(50% - 10px);margin-left:20px"></DatePicker>
                        </FormItem>
                        </Col>
                    </Row>
                    <Row :gutter="32">
            ...
            <Card>
                <div class="table-box">
                    <Table stripe :columns="list.columns" :data="list.data" width="calc(100% - 16px)" @on-selection-change="tableChange">
                        <template slot-scope="{ row, index }" slot="action">
                            <Button type="text" size="small" style="color:#348eed" @click="review(row,index)">预览</Button>
                            <Button type="text" size="small" style="color:#348eed" @click="edit(row)">编辑</Button>
                            <Poptip confirm transfer :title="'确认要删除'+row.name+'么?'" @on-ok="del(row)">
                                <Button type="text" size="small" style="color:red">删除</Button>
                            </Poptip>
                            <Button type="text" size="small" style="color:#348eed" @click="down(row)">下载</Button>
                        </template>
                    </Table>
                </div>
                <div class="page-box">
...
export default {
    ...
    methods: {
        ...
        getList() {
            ...
            params.createTimeStart = this.$comfun.getDateParams(params.createTimeStart,'YYYY-MM-DD HH:mm');
            params.createTimeEnd = this.$comfun.getDateParams(params.createTimeEnd,'YYYY-MM-DD HH:mm');
        ...
        //新增产品型号
        add() {
            this.$refs.modelForm.init({ publicId: this.id });
        },
