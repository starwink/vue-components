...
export default {
  ...
  methods: {
    ...
    handleTagsOption(type) {
      if (type === "close-all") {
        // 关闭所有，除了dashboard
        const res = this.list.filter(item => item.name === "dashboard");
        this.$emit("on-close", res, "all");
      } else {
        // 关闭除当前页和dashboard页的其他页
        const res = this.list.filter(
          item =>
            routeEqual(this.currentRouteObj, item) || item.name === "dashboard"
        );
    ...
    getTagElementByName(name) {
      if (!this.$refs.tagsPageOpened) return;
      this.$nextTick(() => {
